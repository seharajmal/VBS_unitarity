...
char infile[100];
sprintf(infile, "likelihoodScan/%s/%s/LS_objects_k_%s.root",argv[1],argv[2],argv[2]);
...
cout<<"1sigma Confidence Interval = ["<< f1->GetX(y2,-100,0)<< ";" <<f1->GetX(y2,0,100)<<"]"<<endl;
cout<<"2sigma Confidence Interval = ["<< f1->GetX(y1,-100,0)<<";"<<f1->GetX(y1,0,100)<<"]"<<endl;
cout<<"Likelihood Minimum ="<<f1->GetMinimumX(-10,10)<<endl; 
cout<<"Width 1sigma = " << f1->GetX(y2,0,100) - f1->GetX(y2,-100,0) <<endl;
cout<<"Width 2sigma = " << f1->GetX(y1,0,100) - f1->GetX(y1,-100,0) <<endl;
